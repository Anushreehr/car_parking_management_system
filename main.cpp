...
class car
{
	...
	int except(char dname[])
	{
	    int i;
	    int ex;
	    for(int i=0;i<15;i++)
        {
            if(dname[i]=='!'||dname[i]=='@'||dname[i]=='#'||dname[i]=='$'||dname[i]=='%'||dname[i]=='^'||dname[i]=='&'||dname[i]=='*'||dname[i]=='('||dname[i]==')')
            {
                throw ex;
            }
        }
